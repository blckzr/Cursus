      ...
      {isLoading ? (
        ...
        <div className="card p-0 overflow-hidden">
          <div className="grid" style={{ gridTemplateColumns: '60px repeat(5, 1fr)' }}>
            {/* Header row */}
            <div className="bg-beige-50 border-b border-khaki-100" />
            {DAYS.map(d => (
              <div key={d.code} className="bg-beige-50 border-b border-l border-khaki-100 px-3 py-2 text-center">
                <div className="text-[10px] uppercase tracking-wider text-stone-400">{d.code === 'Th' ? 'Thu' : d.label.slice(0, 3)}</div>
                <div className="text-xs font-medium text-stone-700">{d.label}</div>
              </div>
            ))}

            {/* Time gutter */}
            <div className="relative border-r border-khaki-100" style={{ height: SPAN * PX_PER_MIN }}>
              {timeSlots.map(t => (
                ...
                {active.filter(e => meetsOn(e.day_of_week, d.code)).map(e => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {e.room && <div className="text-[10px] text-olive-400 mt-1 leading-tight">{e.room}</div>}
                    </div>
                  );
                })}
              </div>
