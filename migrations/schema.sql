...
--
-- This file replaces the old chain (001–005). Existing migration files are
-- kept for historical reference but are no longer needed.
-- ============================================================
...
-- ============================================================
-- BLOCK SECTIONS  (year/program cohort, e.g. 'BSCS 1-1')
-- ============================================================
...
CREATE INDEX idx_block_sections_program ON block_sections(program_id);
...
CREATE TABLE users (
    ...
    year_level       INT,
    block_section_id UUID        REFERENCES block_sections(id) ON DELETE SET NULL,
    created_at       TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE INDEX idx_users_block_section ON users(block_section_id);
...
CREATE TABLE audit_logs (
    ...
    action      TEXT        NOT NULL,       -- 'UPDATE_SCORE', 'FINALIZE_GRADE', 'PROMOTE_YEAR'…
